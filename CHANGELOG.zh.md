...
- 缓存与分布式能力：`atlas-richie-cache`
- 数据访问与多租户扩展：`atlas-richie-dao` / `atlas-richie-tenant-parent`
- HTTP 客户端统一抽象：`atlas-richie-http-parent`
- 数据脱敏（API/日志/审计场景）：`atlas-richie-desensitize-parent`
