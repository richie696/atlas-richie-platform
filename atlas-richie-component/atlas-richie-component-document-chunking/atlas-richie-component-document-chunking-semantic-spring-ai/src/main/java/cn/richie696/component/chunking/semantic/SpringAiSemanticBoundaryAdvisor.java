...
public final class SpringAiSemanticBoundaryAdvisor implements SemanticBoundaryAdvisor {
  ...
  */
 public SpringAiSemanticBoundaryAdvisor(ChatModel model) { this.model=Objects.requireNonNull(model); }
 /**
  * 让模型为整段文本产出候选语义边界下标。
  * <p>
  * 时序：
  ...
  * @throws NullPointerException 当 {@code ChatResponse} / {@code Generation} / {@code AssistantMessage} 为 {@code null} 时原样抛出。
   */
  @Override public List<Integer> boundaries(String content) {
   ...
   List<Integer> values=new ArrayList<>(); for(String item:output.split(",")) try { int v=Integer.parseInt(item.trim()); if(v>0&&v<content.length()) values.add(v); } catch(NumberFormatException ignored) { }
