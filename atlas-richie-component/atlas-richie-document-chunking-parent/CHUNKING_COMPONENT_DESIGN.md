# atlas-richie-document-chunking-parent 设计说明

## 1. 定位与边界

`atlas-richie-document-chunking-parent` 是独立的通用文档切片组件：将 **文本字符串**按指定规则稳定地转换为 `Chunk`
...
切片核心没有对 `atlas-richie-document-parser`、`atlas-richie-ai` 或 `atlas-richie-vector-parent`
...
```java
// atlas-richie-vector-parent 中的可选模块：
VectorRecord toVectorRecord(Chunk chunk, VectorRecordContext context);
```

该 adapter 应归属 `atlas-richie-vector-parent`，建议命名为 `vector-chunk-adapter`，依赖
...
切片组件不做权限过滤，也不保留业务来源信息。知识库编排层将每个 `Chunk` 映射为 `atlas-richie-vector-parent` 的
`VectorRecord` 时补充：
...
3. **向量适配能力**：在 `atlas-richie-vector-parent` 中实现可选的 `vector-chunk-adapter`，提供 `Chunk -> VectorRecord`
