...
`richie-component-storage-sftp` 是 SFTP（SSH File Transfer Protocol）文件传输的实现，基于 Apache MINA SSHD 3.0 提供 SFTP
文件存储能力。适用于需要通过 SSH 协议访问远程文件服务器的场景。
...
<dependency>
    <groupId>cn.richie696.component</groupId>
    <artifactId>storage-sftp</artifactId>
    <version>${atlas.richie.version}</version>
</dependency>
```
