...
`richie-component-storage-tos` is the implementation of Volcengine Object Storage Service (TOS), built on the Volcengine
TOS SDK to deliver complete TOS storage capabilities. TOS is compatible with the AWS S3 API and supports advanced
features such as image processing and video processing.
...
Each implementation package provides a `StorageEngineProvider` SPI implementation. `TosStorageEngineProvider` is
responsible for:
...
In auto mode, the Provider is registered as a Bean in `TosAutoConfiguration`. In manual mode, it is discovered by the
...
The TOS engine supports client-side direct upload to object storage through presigned URLs, reducing server-side traffic
pressure:
...
<dependency>
    <groupId>cn.richie696.component</groupId>
    <artifactId>storage-tos</artifactId>
    <version>${atlas.richie.version}</version>
</dependency>
```
...
A: TOS is an S3-compatible object storage service. Its API is essentially the same as S3, but storage tiers and
...
- [Direct Upload Policy (DirectUploadPolicy)](../storage-core/README.md#配置模型)
