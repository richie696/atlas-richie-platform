...
`richie-component-storage-tos` 是火山引擎对象存储服务（TOS）的实现，基于火山引擎 TOS SDK 提供完整的 TOS 存储能力。TOS 兼容
AWS S3 API，支持图片处理、视频处理等高级功能。
...
- 根据 `engine: VOLCENGINE_TOS` 配置创建 `TosStorageEngineProvider` Bean
...
- 通过 `registry.switchEngine(StorageEngineEnum.VOLCENGINE_TOS)` 运行时切换
...
每个实现包都提供 `StorageEngineProvider` SPI 实现，`TosStorageEngineProvider` 负责：

| 方法                    | 说明                                                            |
|-------------------------|-----------------------------------------------------------------|
| `supportedEngineType()` | 返回 `StorageEngineEnum.VOLCENGINE_TOS`                         |
| `create(properties)`    | 从配置创建引擎实例                                              |
| `validate(properties)`  | 校验 endpoint / accessKeyId / accessKeySecret / bucketName 必填 |
| `destroy(engine)`       | 释放资源                                                        |

自动模式下 Provider 在 `TosAutoConfiguration` 中注册为 Bean；手动模式下由 Registry 通过 SPI 发现。
...
TOS 引擎支持通过预签名 URL 实现客户端直传到对象存储，减少服务端流量压力：
...
- [核心存储组件 (Core SPI)](../storage-core/README.md)
- [火山引擎 TOS 官方文档](https://www.volcengine.com/docs/6349)
- [TOS Java SDK](https://www.volcengine.com/docs/6349/1099475)
- [直传策略 (DirectUploadPolicy)](../storage-core/README.md#配置模型)
- [存储引擎 SPI (StorageEngineProvider)](../storage-core/README.md)
