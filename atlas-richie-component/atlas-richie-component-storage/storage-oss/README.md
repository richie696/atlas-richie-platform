...
`richie-component-storage-oss` is the Alibaba Cloud Object Storage Service (OSS) implementation, built on the Alibaba
Cloud OSS SDK to provide the full OSS storage capability, including advanced features such as image and video
processing.
...
Each implementation package provides a `StorageEngineProvider` SPI implementation. `OssStorageEngineProvider` is
responsible for:
...
In auto mode, the Provider is registered as a Bean in `OssAutoConfiguration`; in manual mode, it is discovered by the
...
The Alibaba Cloud OSS engine supports client-side direct upload to object storage through presigned URLs, reducing
