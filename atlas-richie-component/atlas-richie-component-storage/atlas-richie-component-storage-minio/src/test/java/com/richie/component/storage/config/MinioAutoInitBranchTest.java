...
/**
 * MinioAutoConfiguration 手工初始化分支测试
 * <p>
 * 覆盖：provider 总是注册、minioAsyncClient 仅在 auto-init=true 时注册。
 */
class MinioAutoInitBranchTest {

    @Test
    void shouldHaveConfigurationAnnotation() {
        assertThat(MinioAutoConfiguration.class.getAnnotation(Configuration.class)).isNotNull();
    }

    @Test
    void providerBeanMethod_shouldBeAnnotatedWithBean() throws NoSuchMethodException {
        Method method = MinioAutoConfiguration.class.getMethod(
                "minioStorageEngineProvider");
    ...
    @Test
    void clientBeanMethod_shouldBeConditionalOnEngineMinio() throws NoSuchMethodException {
        Method method = MinioAutoConfiguration.class.getMethod(
                "minioAsyncClient", StorageProperties.class);
        var conditions = Arrays.stream(method.getAnnotationsByType(ConditionalOnProperty.class))
                ...
                .filter(c -> "minio".equals(c.havingValue()))
                .findFirst();
    ...
    @Test
    void clientBeanMethod_returnTypeShouldBeMinioAsyncClient() throws NoSuchMethodException {
        Method method = MinioAutoConfiguration.class.getMethod(
                "minioAsyncClient", StorageProperties.class);
        assertThat(method.getReturnType()).isEqualTo(MinioAsyncClient.class);
    }
}
