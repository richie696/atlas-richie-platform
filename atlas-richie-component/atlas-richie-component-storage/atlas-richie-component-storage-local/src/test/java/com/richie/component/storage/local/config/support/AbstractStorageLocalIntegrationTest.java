...
@StorageLocalIntegrationTest
@Execution(ExecutionMode.SAME_THREAD)
public abstract class AbstractStorageLocalIntegrationTest {

    private static final String IT_KEY_PATTERN = "it:*";

    @DynamicPropertySource
    static void registerStorageLocalIntegrationProperties(DynamicPropertyRegistry registry) {
        StorageLocalIntegrationTestSupport.getInstance().registerProperties(registry);
    }

    @Autowired
    protected LocalStorageEngine engine;

    @Autowired
    protected FileMetadataMapper fileMetadataMapper;
    ...
    void prepareTestContext() {
        ...
        if (StorageLocalIntegrationTestSupport.getInstance().isExternal()) {
