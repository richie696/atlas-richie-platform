 ...
 * 多租户统一配置模型。
 *
 * <p>对应 YAML 前缀 {@code platform.tenant}。所有可配置项均可通过 Nacos / 环境变量覆盖。
 ...
 */
@Data
@ConfigurationProperties(prefix = MultiTenancyProperties.PREFIX)
public class MultiTenancyProperties {
    ...
    /**
     * 是否允许仅凭未签名的租户请求头建立上下文。默认关闭，只有受控的本地/遗留内部调用
     * 且已在网络层完成可信边界时才应显式开启。
     */
    private boolean allowUnsignedTenantHeader = false;

    /**
     * 直接暴露业务服务时用于独立验证 JWT 的密钥。为空表示不在租户组件重复验证 JWT，
     * 而是使用 Gateway 身份断言或显式允许的内部请求头。
     */
    ...
    /**
     * 租户信息缓存配置（由 core 模块的 {@code CachingTenantInfoProvider} 使用）。
     *
     * <p>默认开启，大幅降低 {@code TenantInfoProvider.getTenantInfo()} 调用频率。
     * 若需关闭，设置 {@code platform.tenant.cache.enabled=false}。</p>
     */
    @NestedConfigurationProperty
    private TenantInfoCacheConfig cache = new TenantInfoCacheConfig();

    /**
     * 启动期 SPI 健康检查配置（由 core 模块的 {@code TenantHealthIndicator} 使用）。
     *
     * <p>默认关闭。生产环境建议开启以确保 {@code TenantInfoProvider}
    ...
    /**
     * Gateway 专属配置。租户总开关由父级 {@link MultiTenancyProperties#enable} 统一控制。
     */
    @Data
    public static class GatewayConfig {

        /**
         * Gateway 与下游租户组件共享的 HMAC 密钥。为空时不生成或接受内部身份断言。
         */
        private String identityAssertionSecret;

        /**
         * Gateway 生成的内部租户身份断言有效期（秒）。
         * 实际有效期还会被访问令牌过期时间截断。
         */
        private long identityAssertionTtlSeconds = 60;
    }
    ...
    public static class TenantInfoCacheConfig {
        /**
         * 是否启用 core 模块的 {@code CachingTenantInfoProvider} 装饰器。
         * 默认 {@code true}（框架自动为 {@code TenantInfoProvider} 叠加本地 TTL 缓存）。
         */
    ...
    public static class HealthCheckConfig {
        /**
         * 是否启用 core 模块的 {@code TenantHealthIndicator}。
         * 启用后若 {@code TenantInfoProvider} 仍是 NoOp 占位，
