 ...
 * <ol>
 *   <li>检查 {@code platform.tenant.ignore-tables} 配置的表是否真实存在(防 typo)</li>
 *   <li>检查 {@code platform.tenant.startup-validation.schema-tables} 配置的表是否含
 *       {@code tenantIdColumn} 列(防业务表缺列导致 SQL 改写失败)</li>
 * </ol>
 *
 * <p><b>仅在 COLUMN 模式下生效</b>。其他模式(SCHEMA/TABLE/DATABASE)无此问题,跳过校验。</p>
 *
 * <p><b>默认关闭</b>({@code platform.tenant.startup-validation.enabled=false})。
 * 业务方需在 application.yml 中显式配置 {@code schema-tables} 列表后启用,
 * 否则启动期无法知道要校验哪些业务表。</p>
 *
 * <p>典型失败信息:
 * <pre>
 * Schema validation failed: Table 'ordrs' listed in platform.tenant.ignore-tables does not exist
 * Schema validation failed: Table 'orders' is missing tenant_id column 'tenant_id' required by platform.tenant
 * </pre>
 *
...
@Component
@Order(1)
@ConditionalOnProperty(prefix = MultiTenancyProperties.PREFIX + ".startup-validation", name = "enabled", havingValue = "true")
...
public class StartupSchemaValidator implements ApplicationRunner {
    ...
    private void validateIgnoreTables(DatabaseMetaData meta) throws SQLException {
        ...
        for (String tableName : ignoreTables) {
            if (!existing.contains(tableName.toLowerCase())) {
                String message = "Table '" + tableName + "' listed in platform.tenant.ignore-tables does not exist";
    ...
    private void validateSchemaTables(DatabaseMetaData meta) throws SQLException {
        ...
        if (schemaTables == null || schemaTables.isEmpty()) {
            log.debug("platform.tenant.startup-validation.schema-tables is empty, skip column check");
            return;
        }
        ...
        for (String tableName : schemaTables) {
            ...
            if (!columns.contains(tenantColumn.toLowerCase())) {
                String message = "Table '" + tableName + "' is missing tenant_id column '"
                        + tenantColumn + "' required by platform.tenant";
                log.error("[多租户] {}", message);
                throw new BusinessException(
