 ...
 */
package cn.richie696.component.tenant.exception;
...
public enum TenantErrorCode {
     ...
     * 租户已被停用。
     */
    TENANT_AUTH_INACTIVE(403, "Tenant account is inactive: {0}"),

    /**
     * 租户仍在初始化，暂不可访问。
     */
    TENANT_PROVISIONING(503, "Tenant {0} is still provisioning"),
    ...
    /**
     * 业务表缺少租户 ID 列 — {@code platform.tenant.tenant-id-column} 配置的列
     * 在 {@code platform.tenant.startup-validation.schema-tables} 列出的某张表中不存在。
    ...
    TENANT_TENANT_ID_COLUMN_MISSING(500,
            "Table {0} is missing tenant_id column '{1}' required by platform.tenant"),

    /**
     * 配置在 {@code platform.tenant.ignore-tables} 的表名在数据库中不存在。
    ...
    TENANT_IGNORE_TABLE_NOT_FOUND(500,
            "Table '{0}' listed in platform.tenant.ignore-tables does not exist in the database");
