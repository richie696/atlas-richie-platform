 ...
 * <p>在 WebFlux 请求入口处完成：
 * <ol>
 *   <li>优先验证 Gateway 签发的内部租户断言</li>
 *   <li>可选：使用配置的 JWT 密钥独立验证 {@code X-ACCESS-TOKEN}</li>
 *   <li>未签名 {@code X-Tenant-ID} header 默认不可信，仅显式开启后作为遗留内部调用降级</li>
...
public class TenantWebFilter implements WebFilter {
    ...
    private Mono<TenantPrincipal> resolveTenant(ServerWebExchange exchange) {
        return Mono.fromCallable(() -> {
            TenantPrincipal principal = resolveFromAssertion(exchange);
            if (principal == null) {
                principal = resolveFromJwt(exchange);
            }
            if (principal == null && properties.isAllowUnsignedTenantHeader()) {
            ...
            if (tenantInfo.getStatus() == null || tenantInfo.getStatus() == TenantStatus.INACTIVE) {
                return Mono.error(new TenantValidationException(
                        TenantErrorCode.TENANT_AUTH_INACTIVE, tenantId));
            }
            if (tenantInfo.getStatus() == TenantStatus.PROVISIONING) {
                return Mono.error(new TenantValidationException(
                        TenantErrorCode.TENANT_PROVISIONING, tenantId));
            }

            String headerTenantId = exchange.getRequest().getHeaders()
                    .getFirst(properties.getTenantIdHeader());
            if (headerTenantId == null || headerTenantId.isBlank()) {
                headerTenantId = exchange.getRequest().getHeaders()
            ...
            if (headerTenantId != null && !headerTenantId.isBlank()) {
                try {
                    ...
                    if (!tenantId.equals(headerId)) {
                        return Mono.error(new TenantValidationException(
                                TenantErrorCode.TENANT_AUTH_MISMATCH, tenantId, headerId));
                    }
                } catch (NumberFormatException ex) {
                    return Mono.error(new TenantValidationException(
                            TenantErrorCode.TENANT_AUTH_INVALID_FORMAT, headerTenantId));
                }
            }
    ...
    private TenantPrincipal resolveFromAssertion(ServerWebExchange exchange) {
