...
public class TenantIdentityFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@Nonnull HttpServletRequest request,
        ...
        // 1. 优先验证 Gateway 签发的内部断言
        TenantPrincipal principal = resolveFromAssertion(request);

        // 2. 直接暴露服务时，可使用本服务配置的密钥独立验证 JWT
        if (principal == null) {
            principal = resolveFromJwt(request);
        }
        ...
        if (tenantInfo.getStatus() == null || tenantInfo.getStatus() == TenantStatus.INACTIVE) {
            writeError(response, TenantErrorCode.TENANT_AUTH_INACTIVE, tenantId);
            return;
        }
        if (tenantInfo.getStatus() == TenantStatus.PROVISIONING) {
            writeError(response, TenantErrorCode.TENANT_PROVISIONING, tenantId);
            return;
        }

        // 7. 断言/JWT tenantId 与 X-Tenant-ID header 交叉校验
