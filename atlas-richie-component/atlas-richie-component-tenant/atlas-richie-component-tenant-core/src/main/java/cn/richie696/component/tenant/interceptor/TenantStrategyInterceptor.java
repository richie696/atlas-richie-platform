 ...
 */
package cn.richie696.component.tenant.interceptor;
...
public class TenantStrategyInterceptor implements Interceptor {
     ...
     * </ol>
     *
     * <p>早返回条件：{@code platform.tenant.enable=false} 或租户上下文未绑定时,直接
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if (principal == null || principal.getTenantId() == null) {
            if (properties.isEnforceAuthTenant()) {
                throw new IllegalStateException(
                        "Tenant context not bound; refusing to execute tenant-aware SQL");
            }
            return invocation.proceed();
        }
        ...
        if (tenantInfo.getStatus() == TenantStatus.MIGRATING) {
            throw new TenantMigratingException(tenantId);
        }
        if (tenantInfo.getStatus() == null || tenantInfo.getStatus() != TenantStatus.ACTIVE) {
