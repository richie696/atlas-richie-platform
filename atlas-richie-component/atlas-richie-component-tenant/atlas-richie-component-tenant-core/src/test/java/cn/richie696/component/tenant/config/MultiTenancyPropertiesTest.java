...
class MultiTenancyPropertiesTest {
    ...
    class Defaults {
        ...
        @Test
        @DisplayName("gateway 配置默认存在且断言有效期为 60 秒")
        void gatewayDefaults() {
            MultiTenancyProperties properties = new MultiTenancyProperties();
            assertThat(properties.getGateway()).isNotNull();
            assertThat(properties.getGateway().getIdentityAssertionTtlSeconds()).isEqualTo(60L);
        }
    }
    ...
    class Setters {
        ...
        @Test
        @DisplayName("新总开关 enable 默认关闭且可独立开启")
        void enableSwitch() {
            MultiTenancyProperties props = new MultiTenancyProperties();
