 ...
 */
package cn.richie696.component.concurrency.config.configuration;
 ...
 *
 * <p>本类不依赖 {@code platform.component.concurrency.async.enabled} 总开关;每个 Bean 独立由各自子系统的
...
public class AlgorithmAutoConfiguration {
     ...
     * 令牌桶限流器 Spring Bean。
     *
     * <p>默认不活跃,通过 {@code platform.component.concurrency.rate-limiter.enabled=true} 激活。
    ...
    @ConditionalOnProperty(prefix = "platform.component.concurrency.rate-limiter", name = "enabled", havingValue = "true", matchIfMissing = false)
     ...
     * 熔断器 Spring Bean。
     *
     * <p>默认不活跃,通过 {@code platform.component.concurrency.circuit-breaker.enabled=true} 激活。
     ...
     */
    @Bean
    @ConditionalOnProperty(prefix = "platform.component.concurrency.circuit-breaker", name = "enabled", havingValue = "true", matchIfMissing = false)
