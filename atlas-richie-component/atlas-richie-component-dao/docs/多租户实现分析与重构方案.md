...
│
└── atlas-richie-component-tenant/               # 🆕 独立的可选租户模块
    ├── strategy/                                   # 策略模式
...
<!-- 不引入 atlas-richie-component-tenant -->
...
| 阶段 | 内容 | 影响范围 |
|------|------|---------|
| **Phase 1** | 创建 `atlas-richie-component-tenant` 模块，将 `tenant/` 下 21 个文件迁移过去 | 新模块 |
...
| **Phase 4** | 更新所有引用 `atlas-richie-component-dao` 并启用租户的业务模块，添加 `atlas-richie-component-tenant` 依赖 | 业务模块 |
