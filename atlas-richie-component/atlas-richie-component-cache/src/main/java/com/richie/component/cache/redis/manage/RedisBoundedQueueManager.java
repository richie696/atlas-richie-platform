...
import com.richie.component.cache.ops.BoundedQueueOps;
...
public class RedisBoundedQueueManager implements BoundedQueueOps {

    private static final String KIND = "BoundedQueue";

    @Qualifier("jsonTemplate")
    private final MultiRedisTemplate<Object> redisTemplate;

    @Override
    public <T> BoundedQueue<T> create(String key, long maxLen, Class<T> clazz) {
        ...
        if (!BoundedListRedisSupport.setMetaIfAbsent(redisTemplate, BoundedListCapacityLimits.metaKey(key), maxLen)) {
            throw new IllegalStateException("BoundedQueue already exists: " + key);
        }
        return new BoundedQueue<>(key, maxLen, clazz, redisTemplate);
    }

    @Override
    public <T> BoundedQueue<T> get(String key, Class<T> clazz) {
    ...
    @Override
    public <T> BoundedQueue<T> getOrCreate(String key, long maxLen, Class<T> clazz) {
        BoundedListCapacityLimits.validateMaxLen(maxLen);
        BoundedQueue<T> existing = get(key, clazz);
        if (existing != null) {
        ...
        BoundedQueue<T> raced = get(key, clazz);
        if (raced == null) {
            throw new IllegalStateException("BoundedQueue meta exists but unreadable: " + key);
        }
    ...
    public boolean grow(String key) {
        BoundedQueue<Object> queue = get(key, Object.class);
        if (queue == null) {
            throw new IllegalArgumentException("BoundedQueue does not exist: " + key);
        }
        return queue.grow();
    }
}
