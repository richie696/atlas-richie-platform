 ...
 *     <td>✅ 业务接口级流量整形：按 {@link RateLimitProperties} 的 {@code path 路由}配置业务规则（VIP / 普通用户 / 敏感接口）</td>
 *   </tr>
 *   <tr>
 *     <td>{@link #circuitBreaker}</td>
 *     <td>{@link CircuitBreakerProperties}</td>
 *     <td>❌ 永不承载反压（同上，反压在容器线程池<strong>之前</strong>处理）</td>
 *     <td>✅ 业务接口级熔断：按 {@link CircuitBreakerProperties} 的 {@code path 隔离}独立 CB 状态机，保护下游资源</td>
 *   </tr>
 *   <tr>
 ...
 *     <td>✅ 业务接口级降级响应：按 {@link DegradeProperties} 的 {@code path 路由}返回业务特定 code/msg</td>
 *   </tr>
 *   <tr>
...
public class WebProperties {
     ...
     *   <li><strong>本子域的正确用法</strong>：按 {@link RateLimitProperties.RouteConfig} 的 {@code routes} path 维度配置业务规则
     ...
     *   <li><strong>本子域的正确用法</strong>：按 {@link RateLimitProperties.RouteConfig} 的 {@code routes} path 维度隔离独立 CB 状态机
     ...
     * 会透传 client header 到 web 端（如 {@code X-Client-Id}），本子域 {@link WebFilterProperties} 的 {@code keyHeader}
     ...
     *   <li><strong>本子域的正确用法</strong>：按 {@link RateLimitProperties.RouteConfig} 的 {@code routes} path 维度返回业务特定的 code/msg
