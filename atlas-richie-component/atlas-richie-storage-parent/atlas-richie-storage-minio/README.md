...
`richie-component-storage-minio` is the MinIO object storage implementation, built on the MinIO Java SDK to provide the
full MinIO storage capability. MinIO is a high-performance, S3-compatible object storage service suited to private cloud
...
Each implementation package provides a `StorageEngineProvider` SPI implementation. `MinioStorageEngineProvider` is
responsible for:
...
In auto mode, the Provider is registered as a Bean in `MinioAutoConfiguration`; in manual mode, it is discovered by the
...
The MinIO engine supports client-side direct upload to object storage through presigned URLs, reducing server-side
traffic pressure:
...
A: MinIO is an S3-compatible object storage service that can be privately deployed, while AWS S3 is a public cloud
service. MinIO's API is fully compatible with S3.

### Q: How do I migrate from S3 to MinIO?

A: Because the APIs are compatible, you only need to change the endpoint and access credentials. No code changes are
required.

### Q: What storage classes does MinIO support?

A: MinIO itself does not differentiate storage classes; all objects use standard storage. Similar functionality can be
achieved through lifecycle policies.
...
A: MinIO uses a distributed architecture and delivers excellent performance, supporting high concurrency. Actual
performance depends on the deployment configuration and hardware resources.

## Related Documentation

- [Core Storage Component (Core SPI)](../storage-core/README.md)
...
- [Direct Upload Policy (DirectUploadPolicy)](../storage-core/README.md#configuration-model)
- [Storage Engine SPI (StorageEngineProvider)](../storage-core/README.md)
