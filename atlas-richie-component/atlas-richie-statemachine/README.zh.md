# Atlas Richie Statemachine组件 (atlas-richie-statemachine)

基于 Easy Rules 4.1.0 构建的轻量级状态机组件，提供统一的状态管理API。支持配置化状态机定义、SpEL 表达式条件判断和动作执行、Redis
持久化存储、Redis Stream 异步数据库同步等特性。
...
- [✨ 功能特性](#✨-功能特性)
    ...
    - [事件与扩展](#事件与扩展)
- [📎 💡 为什么选择状态机](#📎-💡-为什么选择状态机)
    - [传统状态模型 vs 状态机模式对比](#传统状态模型-vs-状态机模式对比)
    - [传统状态模型的典型问题](#传统状态模型的典型问题)
    - [实际业务场景收益](#实际业务场景收益)
    - [迁移成本分析](#迁移成本分析)
    - [总结](#总结)
- [🏗️ 核心架构](#🏗️-核心架构)
...
- [🚀 快速开始](#🚀-快速开始)
    ...
    - [1. 添加依赖](#1-添加依赖)
    - [2. 配置状态机](#2-配置状态机)
    - [3. 创建状态机配置文件](#3-创建状态机配置文件)
    - [4. 定义枚举类型（推荐）](#4-定义枚举类型（推荐）)
    - [5. 使用状态机](#5-使用状态机)
- [📖 详细文档](#📖-详细文档)
    - [核心文档](#核心文档)
    - [技术文档](#技术文档)
...
- [📎 🛡️ 安全模型](#📎-🛡️-安全模型)
    - [SpEL 沙箱机制](#spel-沙箱机制)
    - [深度防御：黑名单机制](#深度防御：黑名单机制)
    - [被阻止的攻击向量示例](#被阻止的攻击向量示例)
    - [为什么选择 SpEL 而非 MVEL](#为什么选择-spel-而非-mvel)
    - [可用变量](#可用变量)
    - [SpEL 语法速查](#spel-语法速查)
- [🎯 最佳实践](#🎯-最佳实践)
    - [1. 状态定义](#1-状态定义)
    ...
    - [4. 存储与持久化](#4-存储与持久化)
    - [5. 错误处理](#5-错误处理)
    - [SYNC 强一致语义](#sync-强一致语义)
    - [6. 订单与支付建议](#6-订单与支付建议)
    - [7. 测试覆盖](#7-测试覆盖)
- [❓ 常见问题](#❓-常见问题)
    - [Q1: 为什么使用 ID 模式而不是传入业务对象？](#q1-为什么使用-id-模式而不是传入业务对象？)
    - [Q2: Redis 和数据库的关系是什么？](#q2-redis-和数据库的关系是什么？)
    - [Q3: 状态机性能如何？](#q3-状态机性能如何？)
    - [Q4: 如何实现终态不可变更？](#q4-如何实现终态不可变更？)
    - [Q5: 如何实现多状态机管理？](#q5-如何实现多状态机管理？)
    - [Q6: SpEL 表达式中如何使用上下文数据？](#q6-spel-表达式中如何使用上下文数据？)
- [📎 📝 更新日志](#📎-📝-更新日志)
    - [v1.0.0](#v100)
...
<dependency>
    ...
    <artifactId>atlas-richie-statemachine</artifactId>
</dependency>
...
<dependency>
    <groupId>cn.richie696.component</groupId>
    <artifactId>atlas-richie-dao</artifactId>
</dependency>
```
