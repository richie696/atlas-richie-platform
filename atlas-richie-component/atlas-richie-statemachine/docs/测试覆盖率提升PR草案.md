# atlas-richie-statemachine 测试覆盖率提升 PR 草案

> 状态： **待审核**
> 模块：`atlas-richie-statemachine`
...
> **注意**:慢日志/详细日志路径 (L127/L129/L161/L163)只影响日志输出,不影响业务行为,且需要 mock 一个慢表达式才能触发,本次
> **不覆盖**。
...
```bash
# 在 atlas-richie-statemachine 目录下
mvn clean verify -q
