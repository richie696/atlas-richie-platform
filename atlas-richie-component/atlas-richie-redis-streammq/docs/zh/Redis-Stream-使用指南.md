...
> **注意（2026-06）**：Stream MQ 已独立为 `atlas-richie-redis-streammq`，门面为 `StreamMQ.stream()`。下文中的
...
- 推荐通过 `StreamMQ.stream().publish(streamKey, payload)` 发布消息（依赖 `atlas-richie-redis-streammq`）。
- 发布时框架会自动：
