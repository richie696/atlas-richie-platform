# Atlas Richie MQTT组件 (atlas-richie-mqtt)
...
RichieMQTT 5.0客户端组件是基于HiveMQ 1.3.7版本开发的商用级MQTT客户端，采用 **事件驱动架构**
设计，专门针对长时间断网后的快速恢复进行了深度优化。组件支持MQTT 5.0协议的所有新特性，包括会话过期、消息过期、主题别名、用户属性、遗嘱消息等。
...
flowchart TB
    ...
    subgraph Connection["连接层"]
        direction LR
        B1["🌐 网络监控<br/>Network Monitor (5s)"]:::conn
        B2["🔍 连接监控<br/>Connection Monitor (10s)"]:::conn
        B3["🔁 快速重连<br/>Fast Reconnect"]:::conn
        B1 --- B2 --- B3
    end

    subgraph Protocol["MQTT 协议层"]
        direction LR
        C1["📋 会话管理<br/>Session Mgmt / Expiry"]:::proto
        C2["📨 消息传递<br/>Delivery / QoS 0-2"]:::proto
        C3["📑 主题订阅<br/>Subscription / Alias"]:::proto
        C1 --- C2 --- C3
    end

    subgraph Richie["RichieMQTT 组件实现"]
        direction LR
        D1["🛡️ 弱网增强<br/>Weak-Network Opt"]:::richie
        D2["📝 手工初始化<br/>Manual Init API"]:::richie
        D3["🔀 网络切换<br/>Public ↔ VPC"]:::richie
        D1 --- D2 --- D3
    end

    subgraph Processing["消息处理层"]
        direction LR
        E1["📤 消息发布<br/>Publish / Retain / Retry"]:::process
        E2["📥 消息订阅<br/>Subscribe / Wildcard"]:::process
        E3["🔀 过滤路由<br/>Filter / Route"]:::process
        E1 --- E2 --- E3
    end
    ...
    subgraph Server["服务器层"]
        ...
        F2["💾 会话存储<br/>Session Store"]:::broker
        F3["📬 消息队列<br/>Message Queue"]:::broker
        F1 --- F2 --- F3
    end

    subgraph Storage["存储层"]
        direction LR
        G1["🗄️ 消息持久化<br/>Persistence"]:::store
        G2["📋 会话状态缓存<br/>Session State"]:::store
        G3["⚙️ 配置存储<br/>Config Store"]:::store
        G1 --- G2 --- G3
    end

    subgraph Security["安全层"]
        direction LR
        H1["🔑 身份认证<br/>Username / Password"]:::security
        H2["🔒 数据加密<br/>TLS / SSL"]:::security
        H3["🛡️ 访问控制<br/>Topic ACL"]:::security
        H1 --- H2 --- H3
    end

    %% 主体数据流：业务消息自上而下
    Client -->|"请求 / 消息"| Connection
    Connection -->|"连接管理"| Protocol
    Protocol -->|"MQTT 协议"| Richie
    Richie -->|"封装增强"| Processing
    Processing -->|"发布 / 订阅"| Server

    %% 存储层与 Broker 的关系
    Server -->|"状态持久化"| Storage
    Storage -.->|"状态回读"| Server

    %% 安全层贯穿
    Security -.->|"防护"| Connection
    Security -.->|"防护"| Protocol
    Security -.->|"防护"| Server
```
...
<dependency>
    ...
    <artifactId>atlas-richie-mqtt</artifactId>
...
**用途**: 手工初始化MQTT客户端 **参数**: 包含完整服务器配置信息的对象 **功能**:
...
**用途**: 带启用标志的手工初始化 **参数**: 服务器配置信息 + 是否启用标志 **功能**: 可以控制是否立即启用MQTT连接
...
**用途**: 动态切换服务器地址 **参数**: 网络类型 + 新主机地址 + 端口 **功能**: 运行时更换MQTT服务器
...
**用途**: 切换网络类型 **参数**: 目标网络类型 **功能**: 在已配置的服务器间切换
...
**用途**: 发布消息 **参数**: 主题 + 消息内容 **功能**: 向指定主题发布消息
...
**用途**: 发布保留消息 **参数**: 主题 + 消息内容 + 是否保留 **功能**: 发布在服务器保留的消息
...
**用途**: 注册普通订阅消息监听器 **参数**: 主题 + 回调函数 **功能**: 订阅主题并处理接收到的消息（支持通配符：`+` 单级通配符，
`#` 多级通配符）
...
**用途**: 注册共享订阅消息监听器（MQTT 5.0 特性） **参数**: 完整的共享订阅主题（格式：`$share/{groupId}/businessTopic`）+
回调函数 **功能**: 订阅共享主题并处理接收到的消息，支持负载均衡和消息分发
...
**用途**: 取消普通订阅消息监听器 **参数**: 主题 **功能**: 取消订阅指定主题
...
**用途**: 取消共享订阅消息监听器 **参数**: 完整的共享订阅主题（格式：`$share/{groupId}/businessTopic`） **功能**:
取消订阅指定共享主题
