...
import cn.richie696.component.tenant.interceptor.DynamicTableNameInnerInterceptor;
import cn.richie696.component.tenant.interceptor.TenantLineInnerInterceptor;
import cn.richie696.component.tenant.interceptor.TenantStrategyInterceptor;
import cn.richie696.component.tenant.monitor.TenantMeterBinder;
import cn.richie696.component.tenant.monitor.TenantMetricsCollector;
 ...
 * <p>多租户上下文（优先使用签名的 {@code X-Tenant-Assertion}，兼容透传
 * {@code X-Tenant-ID}）在微服务间透传需引入对应的通信组件（二选一）：
 * <ul>
 *   <li><b>HTTP</b> — 引入 {@code atlas-richie-microservice}（覆盖 Feign + RestClient）</li>
 *   <li><b>gRPC</b> — 引入 {@code atlas-richie-grpc}（{@code x-tenant-id} 已在默认透传白名单中）</li>
 * </ul>
...
public class TenantAutoConfiguration {
    ...
    static class CoreConfig {
        ...
        public TenantInfoProvider tenantInfoProvider() {
            return new cn.richie696.component.tenant.spi.NoOpTenantInfoProvider();
        }

        /**
         * TTL 缓存装饰器，挂在真实 {@link TenantInfoProvider} 上拦截重复查询。
         * 默认开启（{@code platform.tenant.cache.enabled=true}）。
         * 若关闭则每次 SQL 都直接穿透到业务实现的 {@link TenantInfoProvider}。
         */
        @Bean
        @Primary
        @ConditionalOnProperty(prefix = MultiTenancyProperties.PREFIX + ".cache", name = "enabled", havingValue = "true", matchIfMissing = true)
         ...
         * <ul>
         *   <li><b>HTTP</b> — 引入 {@code atlas-richie-microservice}（覆盖 Feign + RestClient）</li>
         *   <li><b>gRPC</b> — 引入 {@code atlas-richie-grpc}</li>
         * </ul>
        ...
        private void diagnoseCommunicationFrameworks(MultiTenancyProperties props) {
            ...
            if (hasHttp || hasGrpc) {
                ...
                log.warn("    • HTTP (Feign/RestClient) → atlas-richie-microservice");
                log.warn("    • gRPC                    → atlas-richie-grpc");
                log.warn("  若为单体应用，请设置 platform.tenant.microservice=false 关闭此检测");
            }
        }
    ...
    static class ServletConfig {
        ...
        public FilterRegistrationBean<TenantIdentityFilter> tenantIdentityFilter(
            ...
            List<String> superAdminPaths = superAdminPathsProvider.getIfAvailable();
            TenantIdentityFilter filter = new TenantIdentityFilter(
                    properties, tenantInfoProvider, whitelistPaths, superAdminPaths);
    ...
    static class ReactiveConfig {
        ...
        /**
         * WebFlux 环境异常处理器（委托给已有的 {@link cn.richie696.component.tenant.handler.TenantExceptionHandler}）。
         * 已有的 {@code @RestControllerAdvice} 在 WebFlux 下同样生效。
         */
        @Bean
        @ConditionalOnMissingBean(name = "tenantWebFluxExceptionHandler")
        public cn.richie696.component.tenant.handler.TenantExceptionHandler tenantWebFluxExceptionHandler() {
            return new cn.richie696.component.tenant.handler.TenantExceptionHandler();
        }
    }
}
