...
多租户数据访问增强组件（可选），在 `atlas-richie-dao` 基础上为 MyBatis-Plus 增加多租户数据隔离能力。支持
**column / table / schema / database / hybrid** 五种隔离模式，通过配置驱动、策略可插拔，对业务代码零侵入。
...
<dependency>
    ...
    <artifactId>atlas-richie-dao-tenant</artifactId>
</dependency>
```
...
multi-tenancy:
  ...
  table-auto-discover-exclude:           # 排除系统表、迁移历史表等
    - sys_config
    - sys_tenant
    - flyway_schema_history
    - databasechangelog*
  table-templates: [t_order, t_user, t_product]  # 显式列表（与自动发现合并去重）
  datasource:
    shared:
      url: jdbc:postgresql://localhost:5432/shared_db
      username: app
...
> MySQL 不支持 schema 模式。MySQL 中 schema = database，有且只有 1 个，请使用 `mode: database`。
...
> hybrid 模式下每个租户的隔离模式通过 `tenants.<id>.mode` 指定（种子数据），未配置时默认 `COLUMN`。运行期以
> `sys_tenant.isolation_mode` 为权威源。
...
- 事务内禁止切换租户，违规立即回滚
- `@Qualifier` 必需时不得省略
- **跨服务租户传递**：统一通过 `X-Tenant-ID` header（由 `multi-tenancy.tenant-id-header` 配置，默认 `X-Tenant-ID`
  ）传递。出站客户端（HTTP/Feign/gRPC）自动注入，入站 `TenantIdentityFilter` 与 JWT
  交叉校验。详见 [上下文模块 §7](docs/上下文模块详细设计.md)。
...
public class TenantCleanupJob {
    ...
    public void cleanupExpiredTenants() {
        // 无请求上下文 → 需显式绑定平台租户
        TenantPrincipal platformPrincipal = TenantPrincipal.builder().tenantId(0L).build();
        TenantContext.runWithTenant(platformPrincipal, () -> {
    ...
    public void batchProcessForTenant() {
        ...
        tenantInfoProvider.getAllTenantIds().forEach(tenantId -> {
            TenantPrincipal principal = TenantPrincipal.builder().tenantId(tenantId).build();
...
```bash
mvn clean install -pl atlas-richie-component/atlas-richie-dao-tenant -am -DskipTests
```
