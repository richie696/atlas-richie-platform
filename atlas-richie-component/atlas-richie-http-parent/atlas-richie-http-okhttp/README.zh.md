...
http 组件的 **默认 Provider**。基于 [OkHttp 4](https://square.github.io/okhttp/) + `okhttp-sse` + `logging-interceptor`
实现 `HttpClient`。通过 `platform.component.http.provider=okhttp`（默认）激活。
...
| 项           | 值                                                          |
|--------------|-------------------------------------------------------------|
| **坐标**     | `cn.richie696.component:http-okhttp` |
...
完整 API 见 [`http-core`](../http-core/README.zh.md)。

## 🔧 核心能力

### 1. `OkHttpAdapter` —— `HttpClient` 实现

实现 `HttpClient` 的所有方法；`execute()` 把 body 一次性读为 `byte[]` 并释放连接；`async()` / `future()` 提交到 OkHttp
`Dispatcher`。本次请求的 `timeout()` 通过派生 client（覆盖 `readTimeout` / `callTimeout`）实现，不修改全局单例。
...
| **无内置重试 / 熔断**                | 瞬时失败直接抛出              | 与 `atlas-richie-microservice` (microservice has no ZH README) 组合 |
...
本组件通过 `HttpAutoConfiguration#httpComponent` 注册了 `OkHttpClient` Bean。要自定义：优先调整
`platform.component.http.okhttp.*` 属性；或自行声明 `@Bean @Primary OkHttpClient` 覆盖。
...
切到 `BASIC`（生产）或 `HEADERS`（调试）。深度 body 检查请走 [
...
- **门面 core** — [`../http-core/README.zh.md`](../http-core/README.zh.md)
- **其他
  Provider** — [OkHttp](../http-okhttp/README.zh.md) · [HttpClient5](../http-httpclient5/README.zh.md) · [JDK](../http-jdk/README.zh.md) · [RestClient](../http-restclient/README.zh.md)
