# Atlas Richie HTTP OkHttp (http-okhttp)

Default provider for the http component. Implements `HttpClient` on top
of [OkHttp 4](https://square.github.io/okhttp/) + `okhttp-sse` + `logging-interceptor`. Selected by
`platform.component.http.provider=okhttp` (default).
...
- [❓ FAQ](#❓-faq)
    - [Q1: Why does my
      `OkHttpClient` bean override not take effect?](#q1-why-does-my-okhttpclient-bean-override-not-take-effect?)
    ...
    - [Q6: Can I share one
      `HttpClient` across multiple Spring Boot apps?](#q6-can-i-share-one-httpclient-across-multiple-spring-boot-apps?)
...
| Item            | Value                                                       |
|-----------------|-------------------------------------------------------------|
| **Artifact**    | `cn.richie696.component:http-okhttp` |
...
See [`http-core`](../http-core/README.md) for the full API.

## 🔧 Core Capabilities

### 1. `OkHttpAdapter` — `HttpClient` implementation

Every `HttpClient` method is implemented. `execute()` loads body into `byte[]` and releases the connection. `async()` /
`future()` submit to OkHttp `Dispatcher`. Per-request `timeout()` is honored via a derived client with overridden
`readTimeout` / `callTimeout`.

### 2. `OkHttpSseClient` — SSE

Uses `okhttp-sse`'s `EventSourceListener` and feeds events to your `SseListener`. **No auto-reconnect** — implement in
`onFailure` if needed.
...
| **No retry / circuit-breaker built in**          | Transient failures bubble up   | Combine with [`atlas-richie-microservice`](../../atlas-richie-microservice/README.md) |
...
This component already registers an `OkHttpClient` bean via `HttpAutoConfiguration#httpComponent`. Customize via
`platform.component.http.okhttp.*` properties; or declare your own `@Bean @Primary OkHttpClient`.
...
Tune `max-requests` and `max-requests-per-host`. OkHttp pool size is independent — equals
`Runtime.availableProcessors()` by default.
...
Switch to `BASIC` (production) or `HEADERS` (debug). For deep body inspection, route through [
...
- **Core** — [`../http-core/README.md`](../http-core/README.md)
- **Other
  providers** — [OkHttp](../http-okhttp/README.md) · [HttpClient5](../http-httpclient5/README.md) · [JDK](../http-jdk/README.md) · [RestClient](../http-restclient/README.md)
