# Atlas Richie HTTP HttpClient5 (http-httpclient5)

http 组件的 Apache HttpClient 5 Provider。基于 `org.apache.httpcomponents.client5:httpclient5` 与经典
`PoolingHttpClientConnectionManager` 实现 `HttpClient`。通过 `platform.component.http.provider=http_client_5` 激活。
...
- [❓ 常见问题](#❓-常见问题)
    ...
    - [Q2：为什么命中 `ConnectTimeoutException` 而不是预期的
      `responseTimeout`？](#q2：为什么命中-connecttimeoutexception-而不是预期的-responsetimeout？)
    - [Q3：如何挂拦截器？](#q3：如何挂拦截器？)
    - [Q4：跨线程安全吗？](#q4：跨线程安全吗？)
...
| 项           | 值                                                               |
|--------------|------------------------------------------------------------------|
| **坐标**     | `cn.richie696.component:http-httpclient5` |
...
<dependency>
    <groupId>cn.richie696.component</groupId>
    <artifactId>http-httpclient5</artifactId>
</dependency>
```
...
HttpClient5 没有原生 SSE，本模块把响应体当流读取，按行喂入 [
`SseLineParser`](../http-core/README.zh.md#4-sse--全协议支持)，解析后的事件投递到 `SseListener`。
...
通过 `setVersionPolicy(HttpVersionPolicy.NEGOTIATE)` 支持，但 **非**默认。若 HTTP/2 是硬性需求，切到 OkHttp 或 JDK
Provider。
