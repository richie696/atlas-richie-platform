...
| 项           | 值                                                       |
|--------------|----------------------------------------------------------|
| **坐标**     | `cn.richie696.component:http-jdk` |
...
- `buildJdkRequest(HttpRequest)`：通过 `HttpRequestSupport.buildUrlWithParams(...)` 添加 URL 参数，添加 header，`timeout()`
  设为单请求超时，`DELETE` 通过 `builder.method("DELETE", bodyPublisher)` 支持 body。

### 2. `JdkSseClient` —— SSE

用 `BodyHandlers.ofInputStream()` 拿输入流，按行喂入 [
`SseLineParser`](../http-core/README.zh.md#4-sse--全协议支持)。
