# Atlas Richie HTTP RestClient (http-restclient)

http 组件的 Spring `RestClient` Provider。基于 [
...
（Spring 6.1+）实现 `HttpClient`。通过 `platform.component.http.provider=rest_client` 激活。 **复用业务侧提供的任何 Spring
...
| 项           | 值                                                              |
|--------------|-----------------------------------------------------------------|
| **坐标**     | `cn.richie696.component:http-restclient` |
...
<dependency>
    <groupId>cn.richie696.component</groupId>
    <artifactId>http-restclient</artifactId>
</dependency>
```
...
同步路径由 `HttpRequestSupport.executeWithTimeout(...)` 包装；`future(...)` 路径由 `CompletableFuture.orTimeout(...)`
强制超时。
...
不能——`RestClient` 在 `org.springframework.web.client.RestClient`，由 `spring-boot-starter-web` 提供。本 Provider 用
`@ConditionalOnClass(RestClient.class)`，缺类则不创建 Bean。
