...
**Registry 归属**：`RateLimiterRegistry` / `CircuitBreakerRegistry` 抽到 `atlas-richie-component-concurrency`， **不**在
...
**实施策略**： **复用 `atlas-richie-component-concurrency:RateLimiter`**，不自己写 TokenBucket。`RateLimitInterceptor` 仅
50 行包装。
...
**实施策略**： **复用 `atlas-richie-component-concurrency:CircuitBreaker`**，不自己写状态机。`CircuitBreakerInterceptor` 仅
80 行包装。
...
- **依赖与协同平台组件**：
    ...
    - [`atlas-richie-component-oauth`](../atlas-richie-component-oauth/README.zh.md) — 鉴权 / 权限规则（与 web 防护 B
      组互补）。
    ...
    - [`atlas-richie-component-logging`](../atlas-richie-component-logging/README.zh.md) — Service / Controller 方法级日志（与
      web 指标分工）。
