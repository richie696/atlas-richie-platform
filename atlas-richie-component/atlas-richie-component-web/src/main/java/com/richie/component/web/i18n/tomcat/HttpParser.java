...
class HttpParser {
    ...
    /**
     * 去掉字符串首尾双引号并处理内部转义反斜杠。
     *
     * @param input 可能带引号的字符串
     * @return 去引号后的字符串，无效或 null 时返回原值或 null
     */
        ...
        do {
            ...
            if (c == '.') {
                if (octet > -1 && octet < 256) {
                    ...
                    throw new IllegalArgumentException("http.invalidOctet");
                } else {
                ...
                if (octet == -1) {
                    ...
                    if (inIPv6) {
                        throw new IllegalArgumentException("http.invalidLeadingZero");
                    } else {
                ...
                if (inIPv6) {
                    throw new IllegalArgumentException("http.noClosingBracket");
                } else {
                    pos = -1;
                    break;
                }
                ...
                if (inIPv6) {
                    ...
                    throw new IllegalArgumentException("http.closingBracket");
                }
    ...
    static int readHostIPv6(Reader reader) throws IOException {
        ...
        int c = reader.read();
        if (c != '[') {
            throw new IllegalArgumentException("http.noOpeningBracket");
        }
        ...
        do {
            ...
            if (h16Count == 0 && precedingColonsCount == 1 && c != ':') {
                ...
                throw new IllegalArgumentException("http.singleColonStart");
            }
            if (HttpParser.isHex(c)) {
                ...
                h16Size++;
                if (h16Size > 4) {
                    throw new IllegalArgumentException("http.invalidHextet");
                }
            } else if (c == ':') {
                if (precedingColonsCount >= 2) {
                    // ::: is not allowed
                    throw new IllegalArgumentException("http.tooManyColons");
                } else {
                    if (precedingColonsCount == 1) {
                        // End of ::
                        if (parsedDoubleColon) {
                            // Only allowed one :: sequence
                            throw new IllegalArgumentException("http.tooManyDoubleColons");
                        }
                ...
                if (precedingColonsCount == 1) {
                    ...
                    throw new IllegalArgumentException("http.singleColonEnd");
                }
                pos++;
                break;
            } else if (c == '.') {
                if (h16Count == 7 || h16Count < 7 && parsedDoubleColon) {
                    ...
                    throw new IllegalArgumentException("http.invalidIpv4Location");
                }
            } else {
                throw new IllegalArgumentException(
                        "http.illegalCharacterIpv6");
            }
            pos++;
        } while (true);

        if (h16Count > 8) {
            throw new IllegalArgumentException("http.tooManyHextets");
        } else if (h16Count != 8 && !parsedDoubleColon) {
            throw new IllegalArgumentException("http.tooFewHextets");
        }
        ...
        if (c == ':') {
            ...
            throw new IllegalArgumentException("http.illegalAfterIpv6");
        }
    }
        ...
        public DomainParseState next(int c) {
            if (c == -1) {
                if (allowsEnd) {
                    ...
                    throw new IllegalArgumentException("http.invalidSegmentEndState");
                }
