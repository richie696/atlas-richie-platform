# Atlas Richie Microservice Component (atlas-richie-microservice)

> **Microservice infrastructure** component. Bundles **OpenFeign** for declarative HTTP clients, **Resilience4j** /
> Sentinel for circuit breaker + retry, **Spring Cloud LoadBalancer** for service discovery, and **Spring Cloud Gateway**
> helpers. One-stop for inter-service calls.
...
- [❓ FAQ](#❓-faq)
    - [Q1: How is this different from
      `spring-cloud-starter-openfeign`?](#q1-how-is-this-different-from-spring-cloud-starter-openfeign?)
...
| Item                  | Value                                                          |
|-----------------------|----------------------------------------------------------------|
| **Artifact**          | `cn.richie696.component:atlas-richie-microservice`   |
...
- ✅ **Sentinel first** — fail fast on dead hosts; integrates with [
  `atlas-richie-mongodb`](../atlas-richie-mongodb/README.md) and [
  `atlas-richie-http-parent`](../atlas-richie-http-parent/README.md).
...
<dependency>
    ...
    <artifactId>atlas-richie-microservice</artifactId>
</dependency>
```
...
Adds Sentinel integration, platform-aligned configuration, and FeignClient builders that wire `HttpClient` + breaker +
retry in one place.

### `Q2` — `Can` `I` use `Resilience4j` instead of `Sentinel`?

Yes — install `resilience4j-spring-boot3` and configure accordingly. The component does not bind to Sentinel
exclusively.
