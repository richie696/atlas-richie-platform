 ...
 * 3. 自动刷新 CanaryMqttMessageFilter 缓存
 * <p>
...
public class CanaryInstanceManager {
    ...
    /**
     * Gateway 灰度配置（全局总控开关）
     * 统一使用 Gateway 的 DeployConfig，实现全局灰度统一管理
     */
    ...
    public void clearCache() {
        log.info("Clearing MQTT canary filter cache due to configuration change");
        MqttCanaryFilter.clearCache(applicationName);
    }
    ...
    public void onEnvironmentChange(EnvironmentChangeEvent event) {
        ...
        if (event.getKeys().stream().anyMatch(key -> key.contains("platform.gateway.deploy"))) {
            log.info("Gateway canary configuration changed, clearing MQTT canary filter cache. Changed keys: {}", event.getKeys());
            clearCache();
        }
    }
